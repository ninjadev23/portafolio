...
export default function DeviceReal({ type, image, title, fade, onClick }: Props) {
  ...
  if (type === "pc") {
    return (
      <div className="w-64 relative">
        <div
          className={`${baseWrapper} h-52 bg-[url('/pcwallpaper.jpg')] bg-blend-overlay bg-black/80 border-5 border-white rounded-md shadow`}
        >
          <img
            ...
            alt="Project"
          />
          <div className={`${overlay} opacity-0 md:group-hover:opacity-100 md:opacity-0 opacity-100`}></div>
        ...
        <div className="w-full absolute top-1 flex justify-end right-3 bg-black/40 py-1 gap-2">
          <Circle className="w-3 h-3 opacity-40" color="yellow" fill="yellow" />
          <Circle className="w-3 h-3 opacity-40" color="green" fill="green" />
          <Circle className="w-3 h-3 opacity-40" color="red" fill="red" />
        </div>
      </div>
    );
  }
